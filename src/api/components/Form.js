...
const Form = () => {
  ...
  const getUsers = async () => {
    setIsLoading(true);
    axios
      .get('http://172.16.10.132:3574/nc/early_adapters_esxl/api/v1/users', {
        headers: {
          'xc-auth':
  ...
  const handleSubmit = async (e) => {
    try {
      e.preventDefault();
      await axios.post(
        'http://172.16.10.132:3574/nc/early_adapters_esxl/api/v1/users',
        { name: name, email: email, title7: phoneNumber },
        {
          headers: {
            'xc-auth':
              'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJlbWFpbCI6Inl1c2lmLmV5bnVsbGFiZXlsaUBmZXJydW1jYXBpdGFsLmF6IiwiZmlyc3RuYW1lIjpudWxsLCJsYXN0bmFtZSI6bnVsbCwiaWQiOjEsInJvbGVzIjoidXNlciIsImlhdCI6MTY1MjQzNzE1Nn0.pxaIoWn4TxMAZk5vF6DPCaSal9WjmKsQJeJqtugN6FI',
